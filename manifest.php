<?php header('Content-Type: text/cache-manifest'); ?>
CACHE MANIFEST

CACHE:
index.html
assets/bootstrap/css/bootstrap.min.css
assets/css/buttons.css
assets/css/styles.css
assets/fonts/font-awesome.min.css
https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css
https://fonts.googleapis.com/css?family=Lato:400,700,400italic,700italic
https://fonts.googleapis.com/css?family=Montserrat:400,700

assets/fonts/FontAwesome.otf
assets/fonts/fontawesome-webfont.eot
assets/fonts/fontawesome-webfont.svg
assets/fonts/fontawesome-webfont.ttf
assets/fonts/fontawesome-webfont.woff
assets/fonts/fontawesome-webfont.woff2


assets/bootstrap/js/bootstrap.min.js
assets/js/exp_common.js
assets/js/exp_jailbreak.js
assets/js/exp_loader.js
assets/js/pl_mira.js
assets/js/pl_HEN.js
assets/js/pl_updatesDisable.js
assets/js/pl_updatesEnable.js
assets/js/pl_kernelDumper.js
assets/js/pl_historyBlocker.js
assets/js/pl_app2usb.js
assets/js/pl_dumper.js
assets/js/pl_ftp.js
assets/js/pl_linux.js
assets/js/pl_dbBackup.js
assets/js/pl_dbRestore.js
assets/js/pl_ToDEX.js
assets/js/pl_mira_unofficial.js
assets/js/pl_fanthresh.js
assets/js/freelancer.js
assets/js/jquery.min.js

FALLBACK:
/assets/fonts/fontawesome-webfont.woff assets/fonts/fontawesome-webfont.woff
/assets/fonts/fontawesome-webfont.woff2 assets/fonts/fontawesome-webfont.woff2
/assets/fonts/fontawesome-webfont.eot assets/fonts/fontawesome-webfont.eot
/assets/fonts/fontawesome-webfont.svg assets/fonts/fontawesome-webfont.svg
/assets/fonts/fontawesome-webfont.ttf assets/fonts/fontawesome-webfont.ttf
/assets/fonts/FontAwesome.otf assets/fonts/FontAwesome.otf
?>